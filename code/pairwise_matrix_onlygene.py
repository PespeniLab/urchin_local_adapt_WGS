...
# Does a specific position fall in to one or more genes?
# List of lists, there is a list for each of the position in the genotypes variable. Empty if position doesn't fall in a gene.
pos_in_genes = pickle.load(open("data/coords_in_genes.pkl", "rb"))

#####################

### Keeping only positions in "genotypes" that fall into genes
lens=np.asarray([len(i) for i in pos_in_genes])
genotypes=genotypes[:,np.where(lens!=0)]
genotypes=np.squeeze(genotypes)
...
plt.savefig("onlygene_pairwise.png")
...
ax.set_yticks(pos, names)
fig.colorbar(im, ax=ax)
plt.savefig("averaged_onlygene_pairwise.png")
